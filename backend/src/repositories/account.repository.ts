  ...
  async getBalances(userId: string) {
    const { rows } = await pool.query(
      `SELECT a.id, a.name, a.created_at, a.updated_at,
              COALESCE(SUM(
                CASE
                  WHEN me.type = 'income' AND me.account_id = a.id THEN me.amount
                  WHEN me.type = 'transfer' AND me.to_account_id = a.id THEN me.amount
                  WHEN me.type = 'expense' AND me.account_id = a.id THEN -me.amount
                  WHEN me.type = 'transfer' AND me.account_id = a.id THEN -me.amount
                  ELSE 0
                END
              ), 0) AS balance
       FROM accounts a
